...
public class Triangle120 {
  ...
  private int minimumTotalByDAR(List<List<Integer>> triangle, int row, int col, Map<String, Integer> map) {
    ...
    if (col + 1 < currentRow.size()) {
      min = Math.min(min, currentRow.get(col + 1) + minimumTotalByDAR(triangle, row + 1, col + 1, map));
    }
    map.put(key, min);
    return min;
  }
