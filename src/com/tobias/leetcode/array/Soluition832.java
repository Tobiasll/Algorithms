package com.tobias.leetcode.array;


import java.util.Arrays;

/**
 * Given a binary matrix A, we want to flip the image horizontally, then invert it, and return the
 * resulting image.
 *
 * To flip an image horizontally means that each row of the image is reversed.  For example,
 * flipping [1, 1, 0] horizontally results in [0, 1, 1].
 *
 * To invert an image means that each 0 is replaced by 1, and each 1 is replaced by 0. For example,
 * inverting [0, 1, 1] results in [1, 0, 0].
 *
 * Example 1:
 *
 * Input: [[1,1,0],[1,0,1],[0,0,0]] Output: [[1,0,0],[0,1,0],[1,1,1]] Explanation: First reverse
 * each row: [[0,1,1],[1,0,1],[0,0,0]]. Then, invert the image: [[1,0,0],[0,1,0],[1,1,1]]
 */
public class Soluition832 {

  public static void main(String[] args) {
    int[][] arrs = flipAndInvertImage(new int[][]{{1, 1, 0}, {1, 0, 1}, {0, 0, 0}});
    for (int[] a : arrs) {
      System.out.println(Arrays.toString(a));
    }
  }

  public static int[][] flipAndInvertImage(int[][] A) {

    for (int i = 0; i < A.length; i++) {
      for (int x = 0, y = A[i].length - 1; x < y; x++, y--) {
        int temp = Math.abs(A[i][x] - 1);
        A[i][x] = Math.abs(A[i][y] - 1);
        A[i][y] = temp;
      }
      if (A[i].length % 2 != 0) {
        A[i][A.length / 2] = Math.abs(A[i][A.length / 2] - 1);
      }
    }

    return A;
  }
}
